...
<script type="text/javascript">
    $(document).ready(function() {
        var table = $('#PData').DataTable({
            'ajax' : { url: '<?=site_url('settings/get_permission');?>', type: 'POST', "data": function ( d ) {
                d.<?=$this->security->get_csrf_token_name();?> = "<?=$this->security->get_csrf_hash()?>";
            }},
            "buttons": [
            { extend: 'copyHtml5', 'footer': false, exportOptions: { columns: [ 0, 1] } },
        ...
        <div class="col-xs-12">
            ...
            <div class="box-header">
                    ...
                    <ul class="nav navbar-nav pull-right">
                        <a href="<?= site_url('settings/add_user_group'); ?>" title="<?=lang('add_user_group')?>" class='' data-toggle='ajax'>
                ...
                <div class="box-body">
                    <div class="table-responsive">
                        <table id="PData" class="table table-bordered table-hover table-striped">
                            <thead>
                                <tr>
                                    <th class="col-xs-5"><?= lang("name"); ?></th>
                                    <th class="col-xs-5"><?= lang("description"); ?></th>
