...
function loadItems() {
  ...
  if (get('spositems')) {
    ...
    var user          = "<h3>"+lang.user +' : '+ username + "</h3>";
    var pax          = "<h3>"+ 'Pax' +' : '+ $("#total_people").val() + "</h3>";
    ...
    var row_order     ='<thead>'+
                          ...
                          '</tr>'+
                      '</thead>';
    $('.order_span').prepend(style + '' + '<h4>' + '' + '</h4></span>' + queue + pos_customer  + user + pax );
    $('#bill_span').prepend(style + '' + '<h4>' + '' + '</h4></span>' + pos_customer  + time_eating + user  +'<span class="t_items" style="font-weight:bold;font-size: 17px"></span>&nbsp&nbsp&nbsp&nbsp <span class="exchange_rate" style="font-weight:bold;font-size: 17px"></span>');
    ...
    $.each(sortable, function () {
      ...
      var unit_quantity = item.row.unit_quantity;
      ...
      product_discount += formatDecimal(item_discount * unit_quantity, 4);
      ...
      product_tax   += formatDecimal(pr_tax_val * unit_quantity, 4);
      var row_no    = new Date().getTime();
      var modify    = '';
      if (item.row.modify_id !='') {
        var modify  = '<br> <small>Add On : '+ item.row.modify_name +'</small>';
      }
      ...
      if (1 != Settings.remote_printing) {
          if (item.row.unit_quantity-item.row.quantity_print>0) {
            var row_order='<tr class="bb row_' +row_no+ '" data-item-id="' +row_no+ '">'+
                              '<td>#' + (an - 1) + " " + item.row.name + modify + item_comment + "</td>"+
          ...
          if (item.row.sale_type=='void') {
            var row_bill = '<tr class="row_' +row_no+ '" data-item-id="' +row_no+ '">'+
                              ...
                              '<td width="15%" class="text-right">'+ formatMoney((parseFloat(unit_price) + parseFloat(total_modify) + parseFloat(product_tax)) * parseFloat(item.row.unit_quantity)) +'</td>'+
                          '</tr>';
          } else {
            $commit = '';
            if (item_comment !='') {
            ...
            var row_bill = '<tr class="row_' +row_no+ '" data-item-id="' +row_no+ '">'+
                              '<td style="width:140px" class="text-left">'+ (0 ==row_no? t.row.name : item.row.name ) + modify +'</td>'+
                              '<td width="10%" style="white-space: nowrap !important;" class="text-center">' + formatMoney(parseFloat(unit_price) + parseFloat(total_modify)) +'</td>'+
                              '<td height="10%" style="white-space: nowrap !important;" class="text-left">' + item.row.discount +'</td>'+
                              '<td width="10%" style="white-space: nowrap !important;" class="text-center"> x '+Number(item.row.unit_quantity).toFixed(0)+'</td>'+
                              '<td width="10%" class="text-right">' + formatMoney((parseFloat(net_price) + parseFloat(total_modify) + parseFloat(product_tax)) * parseFloat(item.row.unit_quantity)) +'</td>'+
                          '</tr>';
          }
    ...
    $("#discount_remark").val(get("discount_comment"));
    $("#discount_group").val(get("discount_type"));
    ...
    if (count>1) {
      ...
      for (var i = 0; i < currencies.length; i++) {
        ...
        $(".exchange_rate").html("1 $ =   "+formatExchange(rate_exchange.rate,symbol));
      }
...
$(document).ready(function () {
  ...
  // });
   // ==================================Delete 27-09-2023====================================
   // ========================Start Update ON 27-09-2023==========================
  $('#updateDiscount').click(function () {
    var ds               = $('#get_ds').val() ? $('#get_ds').val() : '0';
    var discount_comment = $("#discount_comment").val();
    var discount_type    = $("#discount_type").val();
    store("discount_comment",discount_comment);
    store("discount_type",discount_type);
      if (protect_delete == 1) {
        var boxd = bootbox.dialog({
          title: lang.enter_pin_code,
          closeButton: true,
          message: '<label>Pin Code</label><input id="pos_pin" name="pos_pin" type="password" placeholder="Pin Code" class="form-control kb-pad"> ',
          buttons: {
            danger: {
              label: lang.close,
              className: 'btn-default pull-left',
  ...
  $('#add_discount').click(function () {
    ...
    $('#discount_comment').val(get('discount_comment'));
    if (get("discount_type")) {
       $('#discount_type').select2().select2('val',get('discount_type'));
    }
    $('#get_ds').val(dval);
  ...
  $(document).on('click', '.posdel', function () {
    var row = $(this).closest('tr');
    var item_id = row.attr('data-item-id');
    var quantity = spositems[item_id].row.unit_quantity;
    var price = spositems[item_id].row.real_unit_price;
    if (protect_delete == 1) {
      var boxd = bootbox.dialog({
        title: lang.enter_pin_code,
        closeButton: true,
  ...
  $('#payModal').on('hidden.bs.modal', function (e) {
   ...
   // ====================================Bank Amount====================================
    $('#amount,#bank_amount').change(function (e) {
        var bank_amount = $("#bank_amount").val();
        var total_paying = formatDecimal($('.amount').val()) + formatDecimal(bank_amount);
    ...
    $("#bank_amount").change(function (e) {
        var amount_cash = $("#amount_val").val();
        var bank_amount = $("#bank_amount").val();
        var total_paid  = formatDecimal(amount_cash) + formatDecimal(bank_amount);
        $("#bank_val").val($("#bank_amount").val());
        $("#balance_val").val(formatDecimal(total_paid) - formatDecimal(grand_total));
    });
  // =====================================================================================================
  $.each(currencies, function () {
    var currency = this;
    var rate     = currency.rate;
    $('#amount_'+currency.id+',#bank_amount').change(function (e) {
        var total_paying      = $(".amount").val()  ? $(".amount").val() :0;
        var bank_amount        = $("#bank_amount").val();
        var total_paying_rate = $('.amount_'+currency.id+'').val()/rate;
        total_paying = formatDecimal(total_paying) + formatDecimal(total_paying_rate) + formatDecimal(bank_amount);
        $("#amount_val").val(formatDecimal($(".amount").val()) + formatDecimal(total_paying_rate));
        $('#total_paying').text(formatMoney(total_paying));
        if (Settings.rounding != 0) {
            $('#balance').text(formatMoney(total_paying - round_total));
            $.each(currencies, function () {
