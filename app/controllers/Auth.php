...
class Auth extends MY_Controller {
    ...
    public function create_user() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = [
                ...
                'show_all_record'   => $this->input->post('show_all_record'),
                'edit_price_sell'   => $this->input->post('edit_price_sell')??0,
                'allow_discount'    => $this->input->post('allow_discount')??0,
    ...
    public function getUser() {
        ...
        $this->datatables->select('users.id,users.avatar, users.first_name, users.last_name,users.phone,groups.name as group,users.active,branches.branch_kh as branch,CONCAT(create.first_name ,create.last_name) as created_by');
        $this->datatables->from('users');
        $this->datatables->join('groups','groups.id=users.group_id');
        $this->datatables->join('users create','create.id=users.created_by');
        $this->datatables->join('branches','branches.id=users.branch_id','left');
